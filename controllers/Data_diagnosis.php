...
public $username;
	...
	$this->load->view('module/module_sp/view_tambah_diagnosis', ['model'=>$this->model,
        ...
        // mencetak string 
        $pdf->Cell(190,7,'SISTEM PAKAR MENDETEKSI KERUSAKAN AIR CONDITIONER',0,1,'C');
        $pdf->SetFont('Arial','B',12);
        $pdf->Cell(190,7,'LAPORAN HASIL DETEKSI KERUSAKAN AIR CONDITIONER',0,1,'C');
        // Memberikan space kebawah agar tidak terlalu rapat
        $pdf->Cell(10,7,'',0,1);
        $pdf->SetFont('Arial','B',10);
        $pdf->SetX(22);
        $pdf->Cell(20,6,'NOMOR',1,0,'C');
        $pdf->Cell(70,6,'NAMA KERUSAKAN',1,0, 'C');
        ...
        for($i=0,$no=1;$i<$this->cetak->num_rows();$i++,$no++){
            ...
            $pdf->Cell(70,6,$cetak1->row($i)->nama_kerusakan,1,0,'C');
	...
	$this->session->set_flashdata('tambah', 
                '<div class="alert alert-warning alert-dismissible fade show" role="alert">
                      <strong>ISIKAN JAWABAN ANDA !</strong>
	...
	$query1 = $this->db->query("select * from pengguna where username='$username'");
		...
		$email = $row1->email;
		$query = $this->db->query("select id_kerusakan from pengetahuan where id_gejala='$this->tmp_id'");
		...
		$row = $query->row($i);
		$this->model->id_kerusakan[$i] = $row->id_kerusakan;
		$this->model->id_gejala[$i] = $row->id_gejala;
		}
		$this->model->status[0]=1;
		$this->model->insert_diagnosa();
		$this->model->insert_gejala();
		$this->model->insert_kerusakan();


		if($this->model->rules_p1()==TRUE){
		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P01'");
		$row = $query->row();
		...
		$foto2 = $row->foto2;
		$foto3 = $row->foto3;
		$this->model->id1 = $row->id_kerusakan;
		...
		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P02'");
		$row = $query->row();
		//$query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
		$row1 = $query1->row();
		//$this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
		...
		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P03'");
		$row = $query->row();
		//$query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
		$row1 = $query1->row();
		//$this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
		...
		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P04'");
		$row = $query->row();
		//$query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
		$row1 = $query1->row();
		//$this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
		...
		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P05'");
		$row = $query->row();
		// $query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
		$row1 = $query1->row();
		// $this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
		...
		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P06'");
		$row = $query->row();
		// $query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
		$row1 = $query1->row();
		// $this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
...
// 		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P07'");
// 		$row = $query->row();
// 		//$query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
// 		$row1 = $query1->row();
// 		//$this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
...
// 		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P08'");
// 		$row = $query->row();
// 		//$query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
// 		$row1 = $query1->row();
// 		//$this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
...
// 		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P09'");
// 		$row = $query->row();
// 		//$query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
// 		$row1 = $query1->row();
// 		//$this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
...
// 		$query = $this->db->query("select * from tmp_gejala a join pengetahuan b on a.id_gejala=b.id_gejala join kerusakan c on c.id_kerusakan=b.id_kerusakan join pengguna d on d.id_pengguna=a.id_pengguna where b.id_kerusakan='P10'");
// 		$row = $query->row();
// 		//$query1 = $this->db->query("select min(cf) as min_cf from tmp_gejala");
// 		$row1 = $query1->row();
// 		//$this->model->nilai_cf = ($row1->min_cf*$row->faktor_kepastian)*100;
