...
private $model = NULL;
	...
	public function index(){
		...
		$query1 = $this->db->query("select id_kerusakan from kerusakan");
	$rows1 = $query1->num_rows();
	$this->model->jumlah_kerusakan = $rows1;

	$query2 = $this->db->query("select id_gejala from gejala");
	$rows2 = $query2->num_rows();
	$this->model->jumlah_gejala = $rows2;

	$query3 = $this->db->query("select distinct(id_kerusakan) from pengetahuan");
	...
	$this->load->view('view_profile_admin1', ['model'=>$this->model,
										 	 'jumlah_kerusakan'=>$this->model->jumlah_kerusakan,
